  ...
  ,服务器根据id查找对应的 session 对象，
   ...
   Cookie 被禁用了，session 机制还能实现吗？
    ...
    总结一下目前位置我们所了解的域对象:
     ...
     上述我们设计的 oa 项目遗留问题，就是我们虽然设置的 登录的页面，但是却是可以通过直接访问路径的方式
     直径的跳过我们的登录页面进行了一个直接的访问的。简单是说就是我们的登录页面就是一个摆设而已。
     解决方式：
     就是通过：我们掌握到的 session 会话机制：当我们登录成功之后，我们将用户的登录信息存储到session 当中，
     也就是说session 中如果用户的信息就代表用户的登录成功了。
     如果用户没有登录过，则跳转到登录页面。在访问 “/dept/list” 访问时需要进行一个session 会话的判断，满足即可进入对用的信息路径
     比如：if(session != null && session.getAttribute("username") != null) 这里我们进行了一个双重的判断，
     :需要注意的一点就是：session 是否为null的机制和，因为在我们的 jsp 内置的对象当中存在一个内置已经赋值的 session 对象
     ，我们可以通过指令将 jsp 内置的session 禁用（所谓的禁用了就是，对应的访问生成的 xxx_jsp.java） 当中不会翻译生成其中内置的 session 对象
